...
function renderDelivery(){
  ...
  if(!orders.length){
    el.innerHTML='<div style="text-align:center;padding:30px 20px;color:#999;font-size:14px">Bu bo\'limda buyurtma yo\'q</div>';
    return;
  }
  // Pagination
  const _OP=15;
  ...
  if(_totalPages>1){
    ...
    _pagin=`<div style="padding:12px 0;display:flex;gap:6px;align-items:center;justify-content:center;flex-wrap:wrap">
      <button onclick="setDeliveryPage(${Math.max(1,_myOrdPage-1)})" ${_myOrdPage===1?'disabled':''} style="min-width:34px;height:34px;border-radius:8px;border:1px solid #e5e7eb;background:white;color:#555;font-size:16px;cursor:pointer;font-family:inherit;opacity:${_myOrdPage===1?'.4':'1'}">&#8249;</button>
      ${_btns}
      <button onclick="setDeliveryPage(${Math.min(_totalPages,_myOrdPage+1)})" ${_myOrdPage===_totalPages?'disabled':''} style="min-width:34px;height:34px;border-radius:8px;border:1px solid #e5e7eb;background:white;color:#555;font-size:16px;cursor:pointer;font-family:inherit;opacity:${_myOrdPage===_totalPages?'.4':'1'}">&#8250;</button>
    </div>`;
  }
  el.innerHTML='<div style="padding:8px 0;font-size:13px;font-weight:700;color:#999;margin-bottom:6px">'
...
function renderMyOrders(){
  ...
  if(!myOrders.length){
    el.innerHTML='<div style="text-align:center;padding:40px 20px;color:#999;font-size:14px">Hozircha buyurtma yo\'q</div>';
    return;
  }
  ...
  function orderCard(s){
    ...
    return '<div style="background:white;border-radius:14px;border:0.5px solid #e5e7eb;margin-bottom:8px;overflow:hidden">'
      ...
      +'<div style="font-size:11px;color:#888">'+(ig?ig.name:'')+'</div></div>'
      ...
      +'<div style="display:flex;gap:10px;padding:10px 12px;border-bottom:0.5px solid #f5f5f3">'
      +(s.receiptUrl?'<div style="flex-shrink:0;cursor:pointer" onclick="showFullReceipt(this.querySelector(\'img\').src)"><img src="'+s.receiptUrl+'" style="width:64px;height:64px;border-radius:8px;object-fit:cover;border:0.5px solid #eee"><div style="font-size:10px;color:#999;text-align:center;margin-top:2px">Chek</div></div>'
        :'<div style="width:64px;flex-shrink:0;display:flex;align-items:center;justify-content:center;background:#f9f9f7;border-radius:8px;height:64px;border:1px dashed #e5e7eb"><span style="font-size:22px">🧾</span></div>')
      +'<div style="flex:1;min-width:0">'
      +'<div style="font-size:13px;color:#1a1a1a;margin-bottom:2px">👤 '+(cust.name||'-')+'</div>'
  ...
  if(_totalPages>1){
    ...
    _pagin=`<div style="padding:12px 0;display:flex;gap:6px;align-items:center;justify-content:center;flex-wrap:wrap">
      <button onclick="setMyOrdersPage(${Math.max(1,_myOrdPage-1)})" ${_myOrdPage===1?'disabled':''} style="min-width:34px;height:34px;border-radius:8px;border:1px solid #e5e7eb;background:white;color:#555;font-size:16px;cursor:pointer;font-family:inherit;opacity:${_myOrdPage===1?'.4':'1'}">&#8249;</button>
      ${_btns}
      <button onclick="setMyOrdersPage(${Math.min(_totalPages,_myOrdPage+1)})" ${_myOrdPage===_totalPages?'disabled':''} style="min-width:34px;height:34px;border-radius:8px;border:1px solid #e5e7eb;background:white;color:#555;font-size:16px;cursor:pointer;font-family:inherit;opacity:${_myOrdPage===_totalPages?'.4':'1'}">&#8250;</button>
    </div>`;
  }
  el.innerHTML='<div style="padding:8px 0;font-size:13px;font-weight:700;color:#999;margin-bottom:6px">'
    +(_start+1)+'–'+Math.min(_start+_OP,_total)+' / '+_total+' ta buyurtma</div>'
...
function showFullReceipt(url_or_src){
  ...
  ov.innerHTML='<div style="background:white;border-radius:16px;padding:8px;max-width:90vw;max-height:80vh;display:flex;flex-direction:column;align-items:center"><img src="'+url+'" style="max-width:100%;max-height:70vh;border-radius:10px;object-fit:contain"><div style="font-size:12px;color:#999;margin-top:8px">Yopish uchun bosing</div></div>';
